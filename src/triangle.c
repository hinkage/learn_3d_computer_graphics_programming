...
void draw_texel(int x, int y, uint32_t *texture, vec2_t point_a, vec2_t point_b,
    ...
    float interplated_u = u0 * alpha + u1 * beta + u2 * gamma;
    float interplated_v = v0 * alpha + v1 * beta + v2 * gamma;

    // Map the UV coordinate to the full texture width and height
    int tex_x = abs((int)(interplated_u * texture_width));
    int tex_y = abs((int)(interplated_v * texture_height));
